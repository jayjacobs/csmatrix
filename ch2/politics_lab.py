...
def create_voting_dict(strlist):
    ...
    for line in strlist:
        senator = line.split()
        outdict[senator[0]] = [ int(i) for i in senator[3:len(senator)] ]
    return outdict
...
def find_average_similarity(sen, sen_set, voting_dict):
    ...
    Example:
        ...
        -0.5
    """
    distance = [ policy_compare(sen, k, voting_dict) for k in sen_set ]
    return (sum(distance)/len(distance))

most_average_Democrat = 'Biden' # see notebook
...
def find_average_record(sen_set, voting_dict):
    ...
    out = []
    sens = list(sen_set)
    vlen = len(voting_dict[sens[0]])
    for i in range(vlen):
        out.append(sum([ voting_dict[sen][i] for sen in sen_set ])/len(sens))
    return out
...
def bitter_rivals(voting_dict):
    ...
    outdist = None
    out = ()
    vlen = len(voting_dict)
    vkey = list(voting_dict.keys())
    for first in range(vlen-1):
        for second in range(first+1, vlen):
            dist = list_dot(voting_dict[vkey[first]], voting_dict[vkey[second]])
            if outdist is None or dist < outdist:
                outdist = dist
                out = (vkey[first], vkey[second])
    return out    
